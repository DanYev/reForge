...
def md_nve(sysdir, sysname, runname, nsteps=None):
    mdsys = MmSystem(sysdir, sysname)
    ...
    nsteps = int(TOTAL_TIME / TSTEP) if nsteps is None else nsteps
    logger.info(f"Runnning MD for {nsteps} steps")
...
def extend(sysdir, sysname, runname, until_time=None):    
    """ For NPT runs """
    mdsys = MmSystem(sysdir, sysname)
    mdrun = MmRun(sysdir, sysname, runname)
    logger.info(f"WDIR: %s", mdrun.rundir)
    ...
    simulation.reporters.extend(reporters)
    until_time = 1.2*TOTAL_TIME if until_time is None else until_time * unit.picoseconds
