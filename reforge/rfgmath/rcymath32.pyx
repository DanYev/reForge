"""Cython Math - 32-bit Float Version

Description:
    This module contains internal routines for performing optimized mathematical 
    operations using 32-bit floating point precision. It includes functions for 
    calculating position-position Hessian matrices and perturbation matrices 
    derived from coordinate and covariance data. The computations are accelerated 
    using Cython.

    Note: This module is intended for internal use only within the reForge workflow.
          Use rcmath.py as the public interface.
...
def calculate_hessian(int resnum,
    ...
    x : ndarray of float, 1D
        Array of x coordinates.
    y : ndarray of float, 1D
        Array of y coordinates.
    z : ndarray of float, 1D
    ...
    """
    cdef int i, j
    cdef float x_ij, y_ij, z_ij, r, invr, gamma
    cdef np.ndarray[float, ndim=2] hessian = np.zeros((3 * resnum, 3 * resnum), dtype=np.float32)
...
@timeit
@memprofit
def hessian(np.ndarray[float, ndim=2] vec,
             float cutoff=1.2,
             float spring_constant=1000,
             int dd=0):
    """
    ...
    cdef np.ndarray[float, ndim=2] hessian = np.zeros((3 * n, 3 * n), dtype=np.float32)
...
@timeit
@memprofit
def perturbation_matrix_old(np.ndarray[float, ndim=2] covariance_matrix,
                             int resnum):
    """
    ...
    Parameters
    ----------
    covariance_matrix : ndarray of float, 2D
    ...
    Returns
    -------
    perturbation_matrix : ndarray of float, 2D
    ...
    cdef float norm, sum_val, s
    cdef np.ndarray[float, ndim=2] perturbation_matrix = np.zeros((n, n), dtype=np.float32)
    cdef np.ndarray[float, ndim=2] directions
    cdef float f0, f1, f2
    cdef float delta0, delta1, delta2
...
@timeit
@memprofit
def perturbation_matrix(np.ndarray[float, ndim=2] covariance_matrix, bint normalize=True) -> np.ndarray:
    """
    ...
    cdef float norm, sum_val
    cdef np.ndarray[float, ndim=2] perturbation_matrix = np.zeros((m, n), dtype=np.float32)
    cdef np.ndarray[float, ndim=2] directions
    cdef float f0, f1, f2
    cdef float delta0, delta1, delta2
...
@timeit
@memprofit
def perturbation_matrix_par(np.ndarray[float, ndim=2] covariance_matrix, bint normalize=True) -> np.ndarray:
    """
    Compute a perturbation matrix from a covariance matrix. Same as the old one but no need 
    to specify resnum and works with rectangular matrices
    ...
    cdef int n = covariance_matrix.shape[1] // 3
    cdef int k, i, j, tid
    cdef float sum_val, norm, f0, f1, f2, delta0, delta1, delta2
    ...
    cdef float[:, :, :] local_acc_view = local_acc
    ...
    cdef float[:, :] directions_view = directions
...
@timeit
@memprofit
def perturbation_matrix_iso(np.ndarray[float, ndim=2] ccf, bint normalize=True) -> np.ndarray:
    """
    ...
    cdef np.ndarray[float, ndim=2] perturbation_matrix = np.empty((m, n), dtype=np.float32)
...
@timeit
@memprofit
def perturbation_matrix_iso_par(np.ndarray[float, ndim=2] ccf, bint normalize=True) -> np.ndarray:
    """
...
@timeit
@memprofit
def td_perturbation_matrix(np.ndarray[float, ndim=3] ccf, bint normalize=True) -> np.ndarray:
    """
    ...
    Returns
    -------
    perturbation_matrix : ndarray of float, 3D
...
@timeit
@memprofit
def covmat(np.ndarray[float, ndim=2] X):
    """
    Naive mimicing of:
    
        covmat = np.cov(centered_positions, rowvar=True, dtype=np.float32)
    
    Much slower, as expected.
    """
    cdef int m = X.shape[0]
    cdef int n = X.shape[1]
    cdef np.ndarray[float, ndim=2] cov = np.zeros((m, m), dtype=np.float32)
    cdef int i, j, k
...
@timeit
@memprofit
def pcovmat(np.ndarray[float, ndim=2] X):
    """
